...
export class ListService {
  ...
  private async requireUserByIdentifier(
    identifier: string,
    label: string,
  ): Promise<User> {
    if (!identifier) {
  ...
  private buildReadableListWhere(
    baseWhere: any,
  ...
  async create(createListDto: CreateListDto, currentUserOauthId: string) {
    ...
    const albumIds =
      this.normalizeAlbumIds(
        createListDto.albumIds ?? createListDto.albumList,
      ) ?? [];
  ...
  async findAll(
    ...
    if (userID) {
      ...
      if (
        linkedUserId &&
        excludedOwnerIds.includes(linkedUserId) &&
      ...
      if (viewerUser) {
        ...
        const filteredOwnerIds = Array.from(
        ...
        if (followedIds.length > 0) {
          ...
          where = this.buildReadableListWhere(
            { isSystem: false },
            viewerUser.id,
          );
        } else {
  ...
  async likeList(listId: string, viewerIdentifier: string) {
    const viewer = await this.requireUserByIdentifier(
      viewerIdentifier,
      'Viewer',
    );
  ...
  private async requireListOwner(
    listId: string,
    currentUserOauthId: string,
  ): Promise<AlbumList> {
