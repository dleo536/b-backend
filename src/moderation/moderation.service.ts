...
export class ModerationService {
  ...
  private containsBlockedTerm(value?: string | null): boolean {
    ...
    return this.blockedTerms.some((term) => {
      ...
      return (
        ...
        squashed.includes(term.replace(/\s+/g, ''))
      );
    });
  }
  ...
  private isMissingRelationError(
    ...
    return (
      ...
      driverError.driverError?.message?.includes(
        `relation "${relationName}" does not exist`,
      ) === true
    );
  }

  assertTextFieldsAreAllowed(
  ...
  async getVisibilityExcludedUserIds(
  ...
  async assertUsersCanInteract(
    ...
    const blocked = await this.isBlockedBetweenUsersByIds(
      leftUserId,
      rightUserId,
    );
    if (blocked) {
  ...
  async getBlockState(currentUserOauthId: string, targetIdentifier: string) {
    const currentUser = await this.requireUserByIdentifier(
      currentUserOauthId,
      'Current user',
    );
    const targetUser = await this.requireUserByIdentifier(
      targetIdentifier,
      'Target user',
    );
  ...
  private async resolveReportTarget(
    ...
    switch (targetType) {
      case ReportTargetType.USER: {
        const user = await this.requireUserByIdentifier(
          targetId,
          'Reported user',
        );
        return {
  ...
  async createReport(
    ...
    const reporterUser = await this.requireUserByIdentifier(
      currentUserOauthId,
      'Reporting user',
    );
    const targetType = createReportDto.targetType;
    const targetResolution = await this.resolveReportTarget(
      targetType,
      createReportDto.targetId,
    );

    if (
      targetResolution.ownerUserId &&
      targetResolution.ownerUserId === reporterUser.id
    ) {
  ...
  async listReports(status?: string) {
    ...
    const where =
      ...
      ].includes(normalizedStatus as ReportStatus)
  ...
  async updateReportStatus(
    ...
    const reviewingUser = await this.requireUserByIdentifier(
      currentUserOauthId,
      'Reviewing user',
    );
