...
export class ReviewService {
  ...
  private isReviewPublic(review: Review): boolean {
    return (
      this.normalizeReviewVisibility(review.visibility) ===
      ReviewVisibility.PUBLIC
    );
  }
  ...
  private buildReadableReviewWhere(
    ...
    if (!ownerAccessUserId) {
      return publicConditions.length === 1
        ? publicConditions[0]
        : publicConditions;
    }
  ...
  async create(createReviewDto: CreateReviewDto, currentUserOauthId: string) {
    ...
    const visibility = this.normalizeReviewVisibility(
      createReviewDto.visibility,
    );
    if (!isDraft) {
  ...
  async findAll(
    ...
    let followFilterMode: 'global' | 'following' | 'global-fallback' | 'user' =
      'global';
    ...
    if (userID) {
      ...
      const linkedUserId =
        linkedUser?.id ?? (this.isUuid(userID) ? userID : null);
      if (
        linkedUserId &&
        excludedUserIds.includes(linkedUserId) &&
      ...
      if (viewerUser) {
        ...
        const filteredUserIds = Array.from(
          new Set([...followedIds, viewerUser.id]),
        );
        ...
        if (followedIds.length > 0) {
          ...
          const followedOnlyCount = await this.reviewRepository.count({
          ...
          if (followedOnlyCount > 0) {
            ...
            const filteredTotalCount = await this.reviewRepository.count({
              where: filteredWhere,
            });
  ...
  async findOne(id: string, viewerUid?: string) {
    ...
    const viewerUser = await this.findUserByIdentifier(
      viewerUid?.trim() || undefined,
    );
    const viewerUserId = viewerUser?.id ?? null;
  ...
  async update(
    ...
    if (updateReviewDto.visibility !== undefined) {
      review.visibility = this.normalizeReviewVisibility(
        updateReviewDto.visibility,
      );
    } else {
      review.visibility = this.normalizeReviewVisibility(review.visibility);
    }
  ...
  private async requireReviewAuthor(
    id: string,
    currentUserOauthId: string,
  ): Promise<Review> {
