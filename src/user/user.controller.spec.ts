...
describe('UserController account deletion', () => {
  it('deletes the stored avatar from the backend before removing the user', async () => {
    ...
    expect(userService.removeCurrentUser).toHaveBeenCalledWith(
      'firebase-uid-1',
    );
    expect(firebaseAdminService.deleteUser).toHaveBeenCalledWith(
      'firebase-uid-1',
    );
    expect(
    ...
    expect(
      userService.removeCurrentUser.mock.invocationCallOrder[0],
    ).toBeLessThan(firebaseAdminService.deleteUser.mock.invocationCallOrder[0]);
...
describe('UserController change password', () => {
  it("updates the authenticated user's Firebase password", async () => {
    ...
    expect(userService.findByOauthIdOrThrow).toHaveBeenCalledWith(
      'firebase-uid-1',
    );
