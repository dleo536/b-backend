...
export class UserController {
  ...
  getMyFollowing(@CurrentUser() currentUser: AuthenticatedUser) {
    return this.userService
      .getFollowingByIdentifier(currentUser.uid, currentUser.uid)
      .then((result) => ({
        ...result,
  ...
  getFollowing(
    ...
    return this.userService
      .getFollowingByIdentifier(id, currentUser?.uid)
      .then((result) => ({
        ...result,
        following: toPublicUserResponses(result.following),
      }));
  ...
  update(
    ...
    return this.userService
      .update(uid, updateUserDto, currentUser.uid)
      .then((result) => ({
        ...result,
        user: toSelfUserResponse(result.user),
      }));
  ...
  async removeMe(@CurrentUser() currentUser: AuthenticatedUser) {
    const backendUser = await this.userService.findByOauthIdOrThrow(
      currentUser.uid,
    );
