...
export class FirebaseAdminService {
  ...
  async deleteUser(uid: string): Promise<void> {
    ...
    try {
      ...
      throw new InternalServerErrorException(
        'Could not delete Firebase account',
      );
    }
  }
  ...
  async updateUserPassword(uid: string, password: string): Promise<void> {
    ...
    try {
      ...
      if (authErrorCode === 'auth/user-not-found') {
        this.logger.warn(
          `Firebase user ${uid} was not found for password update`,
        );
      ...
      throw new InternalServerErrorException(
        'Could not update Firebase password',
      );
    }
  }
  ...
  async deleteProfileImage(
    ...
    const storageTargets = this.getProfileImageStorageTargets(
      normalizedUid,
      avatarUrl,
    );
    ...
    for (const storageTarget of storageTargets) {
      try {
        ...
        throw new InternalServerErrorException(
          'Could not delete stored profile image',
        );
      }
    }
  }
  ...
  private parseStorageObjectReference(
    value?: string | null,
    ...
    try {
      ...
      if (parsedUrl.hostname === 'firebasestorage.googleapis.com') {
        const firebaseStorageMatch = parsedUrl.pathname.match(
          /^\/v0\/b\/([^/]+)\/o\/(.+)$/,
        );
