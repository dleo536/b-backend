...
describe('FirebaseAdminService', () => {
  ...
  it('rejects revoked Firebase sessions', async () => {
    ...
    await expect(verificationPromise).rejects.toBeInstanceOf(
      UnauthorizedException,
    );
  ...
  it('deletes the stored profile image for the user from Firebase Storage', async () => {
    ...
    expect(mockStorageBucket).toHaveBeenCalledWith(
      'test-project-id.firebasestorage.app',
    );
    expect(mockStorageFile).toHaveBeenCalledWith(
      'profileImages/firebase-uid-1',
    );
    expect(mockDeleteStorageFile).toHaveBeenCalledWith({
      ignoreNotFound: true,
    });
  });
  ...
  it('deletes a Firebase Storage object referenced by the stored avatar URL', async () => {
    ...
    expect(mockStorageBucket).toHaveBeenNthCalledWith(
      1,
      'test-project-id.firebasestorage.app',
    );
    expect(mockStorageFile).toHaveBeenNthCalledWith(
      1,
      'profileImages/firebase-uid-1',
    );
    ...
    expect(mockStorageFile).toHaveBeenNthCalledWith(
      2,
      'profileImages/firebase-uid-1/thumb.png',
    );
  });
  ...
  it('throws when Firebase Storage cleanup fails unexpectedly', async () => {
    ...
    await expect(
      service.deleteProfileImage('firebase-uid-1'),
